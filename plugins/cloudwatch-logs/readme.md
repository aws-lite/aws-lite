...
Import types from `@aws-lite/cloudwatch-logs`. For plugin autoloading or string plugin names, add `import type {} from '@aws-lite/cloudwatch-logs'` in TypeScript (or `import '@aws-lite/cloudwatch-logs'` in JavaScript) to load the declarations.
